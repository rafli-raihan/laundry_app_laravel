...
<script>
    ...
    <script>
      ...
      function processTransaction() {
        ...
        const customerId = customerSelect.options[customerSelect.selectedIndex].getAttribute("data-id");
        const customerName = customerSelect.value;
        const customerPhone = document.getElementById("customerPhone").value;
        const customerAddress = document.getElementById("customerAddress").value;

        if (!customerId || !customerName || !customerPhone || cart.length === 0) {
        ...
        const details = cart.map(item => ({
            id_service: getServiceId(item.service),
            qty: item.weight,
    ...
    function getServiceId(serviceName) {
        const serviceOption = Array.from(document.getElementById("serviceType").options)
            .find(opt => opt.value === serviceName);
        return serviceOption ? serviceOption.getAttribute("data-id") : null;
    }
